...
var BREWER_BLUE, CUBIC_METERS_FACTOR, EXTENSION, LABEL_PADDING, area, boundingBox, canvasHeight, canvasWidth, drawLineGraph, margin, parseDate, stack, stringToHex, svg, xAxis, xScale, yAxis, yScale,
...
drawLineGraph = function(dataset, dates) {
  var frame, layers, record, storage, storages, value, _i, _j, _len, _len1, _ref;
  ...
  frame.append("text").attr("class", "x label").attr("text-anchor", "end").attr("x", boundingBox.width).attr("y", boundingBox.height - LABEL_PADDING).text("Date");
  ...
  return frame.selectAll(".area").data(layers).enter().append("path").attr("class", "area").attr("d", function(d) {
...
d3.json("monthly-reservoir-storage.json", function(data) {
  var dataset, date, dates, filledDataset, filledRecord, found, interpolatedData, interpolator, localStorages, localTimes, point, record, records, relevantTimes, station, storage, time, times, value, values, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _m, _n, _o, _p, _q, _ref, _ref1, _ref2;
  dates = [];
  _ref = data.dates;
  ...
  filledDataset = [];
  for (_o = 0, _len6 = dataset.length; _o < _len6; _o++) {
    record = dataset[_o];
    filledRecord = {
      "key": record.key,
      "values": []
    };
    for (_p = 0, _len7 = dates.length; _p < _len7; _p++) {
      date = dates[_p];
      found = false;
      _ref2 = record.values;
