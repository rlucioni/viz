...
var BREWER_BLUE, CUBIC_METERS_FACTOR, EXTENSION, IGNORE, LABEL_PADDING, area, boundingBox, canvasHeight, canvasWidth, drawLineGraph, margin, parseDate, stack, stringToHex, svg, xAxis, xScale, yAxis, yScale,
...
drawLineGraph = function(dataset, dates) {
  ...
  frame.append("text").attr("class", "x label").attr("text-anchor", "end").attr("x", boundingBox.width - LABEL_PADDING).attr("y", boundingBox.height - LABEL_PADDING).text("Date").style("fill", "white");
  frame.append("text").attr("class", "y label").attr("text-anchor", "end").attr("y", LABEL_PADDING).attr("dy", ".75em").attr("transform", "rotate(-90)").text("Storage (m³)").style("fill", "gray");
...
d3.json("monthly-reservoir-storage.json", function(data) {
  var count, dataset, date, dates, filledDataset, filledRecord, found, interpolatedData, interpolator, localStorages, localTimes, point, record, records, relevantTimes, station, storage, time, times, truncatedDates, value, values, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _len9, _m, _n, _o, _p, _q, _r, _ref, _ref1, _ref2;
  dates = [];
  _ref = data.dates;
  ...
  for (station in data) {
    values = data[station];
    if (__indexOf.call(IGNORE, station) < 0) {
      records = {
        "key": station,
        "values": []
      };
