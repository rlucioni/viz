...
var BILLION, CUBIC_METERS_FACTOR, EXTENSION, FORCED_COLORS, IGNORE, MILLION, TRUNCATE_DATE, area, boundingBox, canvasHeight, canvasWidth, drawLineGraph, labelPadding, line, margin, parseDate, popAxis, popScale, stack, stringToHex, svg, xAxis, xScale, yAxis, yScale,
...
popAxis = d3.svg.axis().scale(popScale).orient("right").tickFormat(function(d) {
  return d / MILLION;
});
...
drawLineGraph = function(dataset, dates, popData) {
  ...
  frame.append("g").attr("class", "pop axis").attr("transform", "translate(" + boundingBox.width + ", 0)").call(popAxis);
  ...
  line = frame.append("path").datum(popData).attr("class", "pop line").attr("d", line);
  frame.append("text").attr("class", "y label").attr("text-anchor", "end").attr("y", labelPadding.small).attr("dy", ".75em").attr("transform", "rotate(-90)").text("Cubic meters, billions");
  frame.append("text").attr("class", "pop label").attr("text-anchor", "end").attr("y", boundingBox.width - labelPadding.large).attr("dy", ".75em").attr("transform", "rotate(-90)").text("Population, millions");
...
d3.json("monthly-reservoir-storage.json", function(data) {
  ...
  return d3.csv("ca-population.csv", function(popDataRaw) {
    var popData, row, _len10, _s;
    popData = [];
    for (_s = 0, _len10 = popDataRaw.length; _s < _len10; _s++) {
      row = popDataRaw[_s];
      popData.push({
    ...
    return drawLineGraph(filledDataset, dates, popData);
  });
});
